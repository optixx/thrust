...
void startupsliders(int button)
{
    ...
    while (s)
    {
        if (s->active)
        {
            ...
            case 2:
                s->count =
                    (((abs(s->x1 - s->x2) + abs(s->y1 - s->y2) + 1) << 3) - 1) - s->count - 7;
                if (s->count < 0)
                    s->count = 0;
                s->stage = 0;
                break;
...
void bunkerfirebullet(thing* b)
{
    ...
    case 3:
        dir = (dir - 3) & 31;
        newbullet(((*b).x << 3) + 14, ((*b).y << 3) - 68, (int)(32 * cos(dir * M_PI / 16)),
                  (int)(32 * sin(dir * M_PI / 16)), dir >> 1, 0);
        break;
    case 4:
        dir = (dir + 3) & 31;
        newbullet(((*b).x << 3) - 26, ((*b).y << 3) - 68, (int)(32 * cos(dir * M_PI / 16)),
                  (int)(32 * sin(dir * M_PI / 16)), dir >> 1, 0);
        break;
    case 5:
        dir = (dir - 14) & 31;
        newbullet(((*b).x << 3) + 14, ((*b).y << 3) + 44, (int)(32 * cos(dir * M_PI / 16)),
                  (int)(32 * sin(dir * M_PI / 16)), dir >> 1, 0);
        break;
    case 6:
        dir = (dir + 12) & 31;
        newbullet(((*b).x << 3) - 26, ((*b).y << 3) + 44, (int)(32 * cos(dir * M_PI / 16)),
                  (int)(32 * sin(dir * M_PI / 16)), dir >> 1, 0);
        break;
    }
}
