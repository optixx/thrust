...
int game(int demo)
{
    ...
    int level;
    int round;
    static char** levels[LEVELS] = {level1, level2, level3, level4, level5, level6};
    static char textstr[40];
    int localscore;
    options end;
    int ch;
    ...
    while (level < LEVELS && lives > 0 && fuel)
    {
        ...
        while (!endlevel)
        {
            ...
            if (alive && (actionbits & pickup_bit))
            {
                ...
                if (l >= 0)
                    if (resonablefuel((pixx + shipdx + 160) % lenx3, (pixy + shipdy + 88) % leny3,
                                      l))
                    {
#ifndef DEBUG
                        if (!easyrider)
            ...
            restartxy =
                atbarrier((pblockx + ((154 + shipdx) >> 3)) % lenx, pblocky + ((82 + shipdy) >> 3));
            if (restartxy)
            {
